...
// --- NOVO WIDGET DEDICADO A MOSTRAR O PDF (CORRIGE O "PISCA-PISCA") ---
...
class _PdfViewerWithCacheState extends State<PdfViewerWithCache> {
  ...
  void initState() {
    super.initState();
    // A chamada para baixar/buscar o arquivo do cache é feita APENAS UMA VEZ AQUI.
...
class ViolaoScreen extends StatefulWidget {
  const ViolaoScreen({super.key});

  @override
  State<ViolaoScreen> createState() => _ViolaoScreenState();
}

class _ViolaoScreenState extends State<ViolaoScreen> {
  ...
  void _loadAndSetPlaylist() {
    ...
    if (currentQueue.isNotEmpty) {
      final currentInstrument = currentQueue.first.extras?['instrumento'] as String?;
      if (currentInstrument == 'violao') {
  ...
  // --- FUNÇÃO ATUALIZADA PARA USAR O NOVO WIDGET ---
