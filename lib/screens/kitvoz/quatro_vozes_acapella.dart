...
class QuatroVozesAcapellaScreen extends StatefulWidget {
  const QuatroVozesAcapellaScreen({super.key});

  @override
  State<QuatroVozesAcapellaScreen> createState() => _QuatroVozesAcapellaScreenState();
}

class _QuatroVozesAcapellaScreenState extends State<QuatroVozesAcapellaScreen> {
  ...
  void _loadAndSetPlaylist() {
    // Lógica para verificar se a playlist correta já está carregada
    final currentQueue = audioHandler.queue.value;
    if (currentQueue.isNotEmpty) {
      final currentNaipe = currentQueue.first.extras?['naipe'] as String?;
      if (currentNaipe == '4vozesacapella') { // <<<----- MUDANÇA APLICADA AQUI
        print("A playlist de 4 Vozes Acapella já está carregada.");
        return;
      }
    }
    
    print("Carregando ou trocando para a playlist de 4 Vozes Acapella...");
    FirebaseFirestore.instance
        .collection('naipes')
        .doc('4vozesacapella') // <<<----- MUDANÇA APLICADA AQUI
        .collection('musicas')
        .get()
        .then((snapshot) {
      ...
      final mediaItems = validMusicList
          .map((music) => MediaItem(
                id: music.id,
                title: music.titulo,
                extras: {'url': music.url, 'letra': music.letra, 'naipe': '4vozesacapella'}, // <<<----- MUDANÇA APLICADA AQUI
              ))
          .toList();
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      title: 'Kit Voz - 4 Vozes Acapella', // <<<----- MUDANÇA APLICADA AQUI
      body: Container(
